// Simulation-only copy of the supplied RTL/KEYBOARDX/random.sv (VGA_DEMO_Students.qar).
// The supplied file uses SIZE_BITS in its port list before declaring it, which
// Quartus accepts but ModelSim rejects. This copy only moves the parameters into
// a parameter port list; the behaviour is unchanged. Quartus uses the original.
